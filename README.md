...
- **`STFTL1SNRDBLoss` alone is not monotone in reconstruction quality, because the `eps` floor applies per component.** The dynamic-range point above is usually described for quiet targets, but it also bites *per error type*. A DC offset produces almost no imaginary error, so `D1_im` saturates at its floor and pays a fixed reward that any near-purely-real error collects automatically. Measured on `[4, 2, 44100]` stems at amplitude 0.05: a DC offset equal to the signal amplitude scores **-22.4 dB** while 10% white noise scores **-17.6 dB**, even though the time-domain D1 rates the DC error about 10 dB *worse* (+0.96 against -9.13). At DC 1.0 versus 100% noise the inversion is 10.4 dB. This is a property of the published real-plus-imaginary D1 objective, confirmed against an independent reference implementation, not a defect in this code — so it is documented rather than changed. It matters in two places: do not use a bare `STFTL1SNRDBLoss` where a systematic offset or other near-real error is plausible, and do not read the spectral term of a `MultiL1SNRDBLoss` run as a standalone quality metric. At the default `spec_weight=0.5` the combined loss orders both pairs above correctly.
