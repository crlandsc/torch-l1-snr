...
- **Gradient Scaling**: These losses produce gradients a few hundred times larger than a plain L1 loss. That is fine on its own, but a run using the common default gradient-clip threshold of `1.0` would clip away most of a useful gradient. `grad_scale` (default `1.0`, no effect) scales the **gradient** without changing the loss value, so you can size it for clipping without touching what you log. Adam users are unaffected by the scale itself (a constant factor cancels in the update); on plain SGD a non-`1.0` `grad_scale` acts as a learning-rate rescale. If you clip, either raise the threshold to suit this loss or set `grad_scale` below `1.0`. See [gradient magnitude](docs/design_notes.md#gradient-magnitude-and-grad_scale) in the design notes.
