...
## Gradient magnitude and `grad_scale`

**These losses produce gradients a few hundred times larger than a plain L1 loss, and the ratio grows as the estimate converges.** Measured on `[4, 2, 44100]` stems at amplitude 0.05, input-space gradient norm against `torch.nn.functional.l1_loss` on the same batch: about **207x** at 50% relative error, **870x** at 10%, and **3100x** at 1%. A plain L1 gradient is the sign of the error times a constant, so it does not change with error; the SNR log-ratio gradient scales like `1/error`, so it keeps rising as the model improves. The losses in this library stay within about 2x of each other at any given error, so one scale factor serves the whole family.

This is a property of the objective, not a defect, and on its own it does nothing bad: Adam divides by its own gradient scale, so a constant factor cancels in the update. **The one place the raw magnitude bites is gradient clipping.** A run that carries over the common default `clip_grad_norm_(..., 1.0)` from an L1 or MSE recipe will clip almost every step, throwing away most of a useful gradient, and clipping is applied to the *summed* parameter gradient, which is larger still.

`grad_scale` (constructor arg on every loss, default `1.0`) exists to right-size the gradient for clipping. It is an autograd Function that is the identity in the forward pass and multiplies the upstream gradient by `grad_scale` in the backward pass, so **the loss value is bit-exact regardless of `grad_scale`** — logged curves and `state_dict` do not change, only the gradient does. Two ways to keep clipping from eating the gradient:

- Raise the clip threshold to suit this loss (measure the gradient norm on a few batches and set the threshold above it), and leave `grad_scale=1.0`. Preferred if you are unsure.
- Set `grad_scale` below `1.0` to bring the gradient into the range your existing threshold expects.

Caveat for non-Adam optimizers: on plain SGD (or any optimizer without per-parameter scale normalization) a non-`1.0` `grad_scale` is exactly a learning-rate rescale, so fold it into your learning rate rather than tuning both. The default `1.0` is a true no-op — gradients are bit-identical to a build without the knob — so nothing changes for anyone who does not set it.
